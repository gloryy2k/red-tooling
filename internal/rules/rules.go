		...
		Rules: []Rule{
			{
				...
				NotPatterns: []string{`usage:`, `Usage:`, `OPTIONS:`, `--help`, `Could not`, `STATUS_`, `error:`, `Error:`, `unrecognized arguments`, `invalid option`},
				Tag:         "credential",
				Priority:    "high",
				Confidence:  90,
				Mitre:       "T1003",
			...
			},
			{
				...
				NotPatterns:   []string{`failed`, `denied`, `error.*admin`, `Could not`, `STATUS_LOGON_FAILURE`, `STATUS_ACCESS_DENIED`, `NTSTATUS`, `timeout`, `timed out`, `brute`, `Connection refused`, `Authentication failed`},
				Tag:           "privesc",
				Priority:      "critical",
				Confidence:    85,
				Mitre:         "T1078",
			...
			},
			{
				Name:        "sensitive_file",
				Regex:       true,
				Patterns:    []string{`\.kdbx\b`, `\bid_rsa\b`, `\bweb\.config\b`, `(?:^|[\s/\\])\.env(?:\s|$)`, `\b/etc/shadow\b`, `\bSAM\b`, `\bNTDS\.DIT\b`, `\bsecrets\.yml\b`, `\.pfx\b`, `\.p12\b`},
				NotPatterns: []string{`usage:`, `Usage:`, `OPTIONS:`, `--help`, `unrecognized arguments`},
				Tag:         "loot",
				Priority:    "high",
				Confidence:  75,
				Mitre:       "T1005",
